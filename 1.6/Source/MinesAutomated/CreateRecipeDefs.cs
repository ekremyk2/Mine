    ...
    public static class CreateRecipeDefs
    {
        static CreateRecipeDefs()
        {
            try
            {
                ...
                // VTR Optimization: Process resource blocks in batches to avoid frame drops
                var resourceBlocks = Verse.DefDatabase<Verse.ThingDef>.AllDefs
                    .Where(td => td.mineable && td.building?.mineableThing != null &&
                    (td.building.isResourceRock || (settings.enableStoneChunks && td.building.isNaturalRock)))
                    .ToList();
                
                // Process in smaller batches for better performance
                const int BATCH_SIZE = 10;
                for (int i = 0; i < resourceBlocks.Count; i += BATCH_SIZE)
                {
                    var batch = resourceBlocks.Skip(i).Take(BATCH_SIZE);
                    ProcessResourceBlockBatch(batch, settings, recipes);
                }
        ...
        // VTR Optimized batch processing
        private static void ProcessResourceBlockBatch(System.Collections.Generic.IEnumerable<Verse.ThingDef> resourceBlocks, 
            Settings settings, System.Collections.Generic.List<string> recipes)
        {
            foreach (Verse.ThingDef resourceBlock in resourceBlocks)
            {
